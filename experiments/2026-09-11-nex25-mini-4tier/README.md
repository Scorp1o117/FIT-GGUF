...
## APEX vs FIT vs native (2026-09-12)

All three families measured with the identical eval-v1 protocol. APEX-I tiers were
generated by `apex-quant/scripts/generate_config.sh --profile <p> --layers 40` and
quantized with the same runtime and imatrix as the FIT tiers.

| family | tier | size (GiB) | macro KL | same-top |
|---|---|---|---|---|
| FIT | MINI | 12.69 | **0.1860** | 83.92% |
| APEX | Mini | 12.96 | 0.2008 | 83.72% |
| FIT | COMPACT | 13.76 | **0.1495** | 85.81% |
| FIT | BALANCED | 15.57 | **0.0998** | 89.32% |
| APEX | I-Compact | 15.75 | 0.1006 | 89.23% |
| APEX | I-Quality | 21.48 | 0.0468 | 93.03% |
| FIT | QUALITY | 23.12 | **0.0465** | 93.36% |
| APEX | I-Balanced | 23.83 | **0.0404** | 93.66% |

![APEX vs FIT vs native](apex-vs-fit-vs-native.png)

### What the curve says

1. **Both families beat the native preset ladder at matched size — that is the
   bar that matters.** At ~15.6 GiB the native ladder gives Q3_K_M **0.1615**
   while FIT BALANCED gives **0.0998** and APEX I-Compact **0.1006**: a 38%
   KL reduction for the same file size. Same story at 13.8 GiB (FIT COMPACT
   0.1495 vs native IQ3_M 0.1498 at 14.38 GiB — FIT reaches it 0.6 GiB sooner)
   and at 23 GiB (FIT 0.0465 / APEX 0.0468 vs native Q5_K_M 0.0586).
2. **The families split by size, so neither dominates the curve.**
   * ≤ 16 GiB: **FIT wins.** FIT MINI 0.1860 @ 12.69 GiB ties native IQ3_XXS and
     beats APEX Mini (0.2008 @ 12.96 GiB); FIT BALANCED edges APEX I-Compact at
     the same size (0.0998 @ 15.57 vs 0.1006 @ 15.75).
   * ≥ 21 GiB: **APEX wins on size.** APEX I-Quality matches FIT QUALITY's KL
     (0.0468 vs 0.0465) while being **1.6 GiB smaller**, and I-Balanced reaches
     0.0404 — the best KL per byte of any artifact measured here.
3. **Two tiers have no counterpart in the other family**: APEX I-Balanced
   (23.83 GiB) and FIT COMPACT (13.76 GiB, which native only matches at
   IQ3_M's 14.38 GiB). A release covering the whole range would carry both
   families' points rather than picking a winner.
4. FIT's numbers here reproduce the search's reported values to the last digit
   (0.046503 / 0.099753 / 0.149503 / 0.186040), which independently confirms the
   search harness.

Raw data: `apex-vs-fit-eval.json` (per-domain KL, same-top, sizes).
Chart: `apex-vs-fit-vs-native.png`. Scripts: `scripts/eval_artifact.py`,
`scripts/plot_apex_vs_fit.py`.
