...
### P4 — 新模型验收（冒烟测试）🚧 进行中
- [x] imatrix + 5 域 BF16 参照已生成（2026-09-06 首次尝试）
- [ ] 单条 `fit calibrate` 命令跑通全流程（无模型专用脚本、无手工干预）
- [ ] 产出合法 bundle（candidate 或 validated 都算通过）
...
### C3 + 发布
- [ ] 命名规范定稿
- [ ] README 补 `fit calibrate` / `fit registry` 用法
- [ ] `pyproject.toml` → 0.3.0 + CHANGELOG
...
- 2026-09-06 22:46：P2 Gate B ✅（Spark fresh 八件套产出）；P4 启动后中断（imatrix + 5 域参照 + IQ2_XXS 量化与首域评测完成，进程被掐断）。
