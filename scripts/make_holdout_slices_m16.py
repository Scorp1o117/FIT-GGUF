...
OUT_DIR = EVAL_DATA / "holdout-m16"
RECORD = Path(
    "/run/media/s117/OS/FIT-GGUF/experiments/2026-08-29-m16-granite-reveal/holdout6-slices.json"
)
SLICE_CHARS = 65_536

SOURCES = {
    "wiki_test": (EVAL_DATA / "wikitext-2-raw/wiki.test.raw", {"m9": 0, "m11": 655_360, "m13": 983_040, "m14": 327_680, "m15": 458_752}, 1_114_112),
    "wiki_valid": (EVAL_DATA / "wikitext-2-raw/wiki.valid.raw", {"m9": 0, "m11": 655_360, "m13": 983_040, "m14": 327_680, "m15": 458_752}, 131_072),
    "chinese": (CALIB / "combined_cn_medium.parquet", {"m9": 3_173_914, "m11": 4_000_000, "m13": 6_500_000, "m14": 7_700_000, "m15": 8_800_000}, 1_500_000),
    "code": (CALIB / "code_medium.parquet", {"m9": 12_536_883, "m11": 18_000_000, "m13": 28_000_000, "m14": 21_000_000, "m15": 33_500_000}, 8_000_000),
    "agent_chat": (CALIB / "agentworld_clean_quick.txt", {"m9": 998_087, "m11": 400_000, "m13": 1_200_000, "m14": 1_400_000, "m15": 1_500_000}, 100_000),
}
PRIOR_FILES = {
...
def main() -> None:
    ...
    for domain, (src, prior_offsets, m16_off) in SOURCES.items():
        text = load_source(src)
        assert m16_off + SLICE_CHARS <= len(text)
        for label, off in prior_offsets.items():
            assert m16_off >= off + SLICE_CHARS or m16_off + SLICE_CHARS <= off, f"{domain} overlaps {label}"
        hold = text[m16_off : m16_off + SLICE_CHARS]
        leaked = [
            f"{label}:{w[:24]!r}"
